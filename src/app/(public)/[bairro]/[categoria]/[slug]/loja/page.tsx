...
import { StoreFront, type StoreProduct } from "./_components/storefront"
...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  const business = await db.business.findUnique({ where: { slug }, select: { name: true, storeTagline: true, storeCoverUrl: true } })
  if (!business) return {}
  return {
    ...
    openGraph: { title: `Loja de ${business.name}`, images: business.storeCoverUrl ? [business.storeCoverUrl] : undefined },
  }
}
    ...
    <main className="pb-24">
      ...
      <section className="relative">
        <div className="relative h-[44vh] min-h-[320px] max-h-[480px] w-full overflow-hidden bg-flora-deep">
          ...
          {/* Voltar */}
          <Link href={profileUrl} className="absolute top-5 left-4 sm:left-6 inline-flex items-center gap-1.5 text-sm text-white/90 hover:text-white bg-black/20 backdrop-blur-sm px-3 py-1.5 rounded-full transition-colors">
