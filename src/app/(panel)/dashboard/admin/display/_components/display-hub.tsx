...
import { Search, QrCode, Printer, X, Loader2, Store } from "lucide-react"
import { DisplayCard, type DisplayCardData } from "@/components/display-card"

interface Result { id: string; name: string; category: string; display: DisplayCardData }
  ...
  return (
    <div className="space-y-6">
      <div className="rounded-2xl border border-gray-100 dark:border-white/[0.07] bg-white dark:bg-white/[0.02] p-5 space-y-4">
        <div>
          <p className="font-semibold dash-title">Display de um negócio</p>
          <p className="text-xs dash-muted">Busque pelo nome e gere o display (cartaz A6 com QR) de qualquer lojista.</p>
        </div>
          ...
          <div className="divide-y divide-gray-50 dark:divide-white/[0.04] rounded-xl border border-gray-100 dark:border-white/[0.07] overflow-hidden">
            {results.map(r => (
              <button key={r.id} onClick={() => setSelected(r)}
                ...
                <QrCode className="w-4 h-4 dash-muted flex-shrink-0" />
              </button>
            ))}
          </div>
        )}
      ...
      {selected && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4" style={{ background: "rgba(0,0,0,0.5)", backdropFilter: "blur(4px)" }} onClick={() => setSelected(null)}>
          <div className="w-full max-w-sm bg-white dark:bg-[#0f1c18] rounded-2xl p-5 space-y-4 max-h-[90vh] overflow-auto" onClick={e => e.stopPropagation()}>
            <div className="flex items-start justify-between gap-3">
              <div>
                <h3 className="font-serif text-lg font-semibold dash-title flex items-center gap-2"><QrCode className="w-5 h-5 text-emerald-500" /> Display da loja</h3>
                <p className="text-xs dash-muted mt-0.5">{selected.name}</p>
              </div>
              <button onClick={() => setSelected(null)} className="p-1.5 rounded-lg hover:bg-gray-100 dark:hover:bg-white/10"><X className="w-4 h-4 dash-muted" /></button>
            </div>
            <div className="flex justify-center">
              <div style={{ width: "252px", height: "355px", overflow: "hidden", borderRadius: "8px" }}>
                <div style={{ transform: "scale(0.635)", transformOrigin: "top left" }}>
                  <DisplayCard data={selected.display} />
                </div>
              </div>
            </div>
            <Link href={`/display-print/${selected.id}`} target="_blank" rel="noopener noreferrer"
